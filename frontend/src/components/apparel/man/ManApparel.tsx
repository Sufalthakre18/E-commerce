...
const ManApparel: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const manApparelProducts = data.products.filter(
          (product) => product.category.parent?.name === 'Apparel' && product.category.name === 'Man-Apparel',
        );
        setProducts(manApparelProducts);
      } catch (err) {
  ...
  // No products state (added - same as Assets page)
