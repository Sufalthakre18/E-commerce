...
export default function AdminOrdersPage() {
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (isFilterActive) {
          ...
          if (productName.trim()) queryParams.append('productName', productName);
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter(order => {
      ...
      const matchesTotalMin =
        !filters.minTotal || order.total >= Number(filters.minTotal);
      const matchesTotalMax =
        !filters.maxTotal || order.total <= Number(filters.maxTotal);

      return (
        matchesStatus &&
        matchesProductOrOrderId &&
        matchesDate &&
        matchesTotalMin &&
        matchesTotalMax
      );
  ...
  const handleConfirmReturn = async (orderId: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/return/confirm/${orderId}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${getAuthToken()}`,
        },
      ...
      if (res.ok) {
        ...
        setOrders(prev =>
          prev.map(order => (order.id === orderId ? { ...order, status: 'RETURNED' } : order))
        );
      } else {
  ...
  const hasActiveFilters = filters.status !== 'ALL' || filters.productName || filters.from || filters.to || filters.minTotal || filters.maxTotal;
        ...
        {filteredOrders.length === 0 ? (
          ...
          <div className="space-y-4">
            {filteredOrders.map(order => (
              ...
              <div className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 hover:border-indigo-200 cursor-pointer group" key={order.id}>
                <div className="p-5">
                  <Link href={`/admin/orders/${order.id}`} key={order.id}>
                    ...
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="text-right">
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${order.status || 'bg-gray-100 text-gray-800 border-gray-200'}`}>
                          {order.status}
                        </span>
                      </div>
                    </div>
                    ...
                    {order.address && (
                      <div className="mb-4 p-3 bg-blue-50 rounded-lg">
                        ...
                        <div className="text-sm text-blue-900">
                          {order.address.line1}, {order.address.line2}, {order.address.city}, {order.address.state}, {order.address.country} {order.address.postalCode}
                        </div>
