...
import { Package, Plus, Search, Filter, TrendingUp, AlertCircle, Edit, Trash2, Eye, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  const fetchProducts = async (page: number) => {
    ...
    try {
      ...
      const data: ApiResponse = await fetchWrapper(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/products?${params.toString()}`
      );
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6 mt-4 sm:mt-6">
            <StatsCard icon={<Package className="w-5 h-5 sm:w-6 sm:h-6 text-white" />} label="Total Products" value={pagination.total} color="from-indigo-500 to-indigo-600" />
        ...
        {products.length === 0 ? (
            ...
            <div className="mt-6 flex justify-center">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`p-2 rounded-lg ${pagination.page <= 1 ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 hover:bg-indigo-100'}`}
                >
                ...
                <button
                  ...
                  className={`p-2 rounded-lg ${pagination.page >= pagination.totalPages ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 hover:bg-indigo-100'}`}
                >
          ...
          <>
            <ProductTable products={products} handleDelete={handleDelete} deleteLoading={deleteLoading} />
            {/* Pagination Controls */}
            <div className="mt-6 flex justify-center">
              <div className="flex items-center gap-2">
                <button
